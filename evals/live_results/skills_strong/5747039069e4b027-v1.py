def extract(text_layout: dict) -> dict:
    ...
    vendor_name_match = re.search(r'Bill From: (.+?),', full_text)
    ...
    invoice_date_match = re.search(r'Dated: (\d{2}/\d{2}/\d{4})', full_text)
    ...
    line_items = re.findall(r'^.+? \d+ x \d+\.\d+ \d+\.\d+$', full_text, re.MULTILINE)
