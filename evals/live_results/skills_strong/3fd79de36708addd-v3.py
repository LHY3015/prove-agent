def extract(text_layout: dict) -> dict:
    ...
    invoice_date_match = re.search(r'Issued (.+?)\n', full_text)
    ...
    total_match = re.search(r'Total Due (\d+\.\d+)', full_text)
    total = total_match.group(1) if total_match else ''
    line_items = full_text.split('Item Qty Rate Line Total\n')[1].split('Subtotal')[0].strip().split('\n')
