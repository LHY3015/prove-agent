def extract(text_layout: dict) -> dict:
    ...
    invoice_number_match = re.search(r'Number (\w+-\d+)', full_text)
    invoice_number = invoice_number_match.group(1) if invoice_number_match else ''
    
    invoice_date_match = re.search(r'Issued (\w{3} \d{2}, \d{4})', full_text)
    ...
    total_match = re.search(r'Total Due (\d+\.\d{2})', full_text)
    total = total_match.group(1) if total_match else ''
    
    line_items = full_text.split('Item Qty Rate Line Total')[1].split('Subtotal')[0].strip().split('\n')
