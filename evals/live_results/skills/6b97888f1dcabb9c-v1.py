def extract(text_layout: dict) -> dict:
    ...
    invoice_date_match = re.search(r'Date:\s+(\d{4}-\d{2}-\d{2})', full_text)
    ...
    tax_match = re.search(r'Tax \(\d+\.\d+%?\)\s+(\d+\.\d{2})', full_text)
    tax = tax_match.group(1) if tax_match else ''
    
    total_match = re.search(r'Total\s+(\d+\.\d{2})', full_text)
    total = total_match.group(1) if total_match else ''
    
    line_items = re.findall(r'^[^S].*?\s+\d+\s+\d+\.\d{2}\s+\d+\.\d{2}$', full_text, re.MULTILINE)
